    ...
    if(isset($_POST['submid']))// khi nhân submit thì sẽ thực thi các lệnh sau
    {
     //INSERT INTO `manage`(`idmanage`, `hovaten`, `tendangnhap`, `matkhau`, `rematkhau`, `email`, `sdt`) VALUES (
      ...
      }else
      {
          ...
          //thực thi các lệnh 
          $sql = "UPDATE `manage` SET `hovaten`='$hovaten',`tendangnhap`= '$tendangnhap' ,`matkhau`= '$matkhau',`rematkhau`= '$rematkhau',`email`= '$email',`sdt`= '$sdt' WHERE idmanage = $id";
          $query = mysqli_query($conn,$sql);
          if($query)
          {
                echo '<script>alert("Bạn đã sửa thành công ");</script>';
              //header('location: login_customer.php');
              echo '<script type="text/javascript">window.location.replace("http://localhost:8080/hoangdinhhanhdean/manage/listadmin.php")</script>';
    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-md-4">
                ...
                <input type="text" class="form-control" name="hovaten" placeholder="Full name" value="<?=$pro['hovaten']?>">
                <input type="text" class="form-control" name="tendangnhap" placeholder="User name" value="<?=$pro['tendangnhap']?>">
                <input type="text" class="form-control" name="matkhau" placeholder="Password" value="<?=$pro['matkhau']?>">
                <input type="text" class="form-control" name="rematkhau" placeholder="Enter the password" value="<?=$pro['rematkhau']?>">
                <input type="email" class="form-control" name="email" placeholder="E-mail" value="<?=$pro['email']?>">
                <input type="text" class="form-control" name="sdt" placeholder="Number Phone " value="<?=$pro['sdt']?>"> 
                <button type="submit" class= "btn btn-info" name="submid" >Change</button>
